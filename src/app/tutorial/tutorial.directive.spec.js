...
describe("Directive: tutorial", function() {
  ...
  beforeEach(module("wordCloudChrome"));
  beforeEach(module("wordCloudChromeTemplates"));
  var el, scope, compileFunction;

  beforeEach(inject(function($rootScope, $compile) {
    el = angular.element("<div data-tutorial-slider json='tutorial2'></div> <div data-tutorial-slider json='tutorial1'></div>");
    scope = $rootScope.$new();
    scope.tutorial1 = ["tutorialExplanationSlide", "tutorialBeginSlide", "tutorialEditSlide", "tutorialFinishSlide"];
    scope.tutorial2 = ["tutorialExplanationSlide", "tutorialBeginSlide", "tutorialEditSlide", "tutorialFinishSlide"];
  ...
  it("should make a tutorial element with contents from scope", function() {
    ...
    inject(function() {
      ...
      expect(angular.element(el.find("h1")).text().trim()).toEqual('Welcome to iLanguageCloud!');
      expect(angular.element(el.find("h2")).text().trim()).toEqual('Begin');
      expect(angular.element(el.find("h3")).text().trim()).toEqual('Edit');
      expect(angular.element(el.find("h4")).text().trim()).toEqual('Finished?');
    });
  });
